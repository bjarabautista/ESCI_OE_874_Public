...
#           the geodetic reference frame at reception
        R_rx = list()
#         create a number of np.zeros matrices of dimension 3xn_twtt_times:
        ...
        t_mru = ([e.timestamp() for e in motions.times])
        t_wl = ([e.timestamp() for e in water_levels.times])
...
#         "x and y are arrays of values used to approximate some function f: y = f(x). This class returns a function whose call method uses interpolation to find the value of new points."
        ...
        for t in t_twtt:
            ...
            # A8.2.6  Looping through the TWTTs - Leave in place -> don't belive
            ping += 1
